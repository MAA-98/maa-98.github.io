...
"A computation therefore consists of an initial lambda expression \[...\] plus a finite sequence of lambda terms, each deduced from the preceding term by one application of beta reduction." ([HPFP](#HPFP), p. 36) Note that there is a choice of reduction strategy by which applications one chooses to reduce first, discussed later.
