...
The simple von Neumann architecture stores program instructions and data in the same address space. Similarly, a Unix process provides a virtual address space of one long contiguous memory, which C programs then make use of with pointers.

## Basics

Preliminaries on memory:
...
A typical Unix process address space is organized roughly as follows: (APUE Sec 7.6)
...
Heap allocation is usually more expensive than stack allocation because of additional bookkeeping and having to sometimes request memory from the OS through system calls, which are slower than in program instructions.
...
Since an object like a 64-bit unsigned integer (`uint64_t`) occupies 8 bytes, advancing a pointer of such elements will increment it by 8. On ARM AArch64 and macOS, `malloc` (APUE Sec 7.8) returns 16-byte aligned pointers, so their lowest 4 bits are always zero.
